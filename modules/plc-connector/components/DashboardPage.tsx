  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">PLC Connector</h1>
          <p className="text-gray-400 text-sm">Automatikus adatgyűjtés PLC vezérlőkből</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs mb-1">Összes eszköz</p>
          <p className="text-2xl font-bold text-white">{devices.length}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs mb-1">Online</p>
          <p className="text-2xl font-bold text-green-400">{online}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs mb-1">Offline</p>
      ...
      {/* Device list */}
      {loading ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center text-gray-500">Betöltés...</div>
      ) : devices.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center text-gray-500">
          Nincs regisztrált PLC eszköz. Kattints az &quot;Új eszköz&quot; gombra.
        </div>
      ) : (
          ...
          {devices.map(d => (
            <div key={d.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-green-500/50 transition-colors">
              ...
              <div className="space-y-1 text-sm text-gray-400">
                <p>Protokoll: <span className="text-gray-300">{d.protocol.toUpperCase()}</span></p>
                <p>IP: <span className="text-gray-300">{d.ipAddress}:{d.port}</span></p>
                {d.rack !== null && <p>Rack/Slot: <span className="text-gray-300">{d.rack}/{d.slot}</span></p>}
                {d.lastSeenAt && <p>Utolsó aktivitás: <span className="text-gray-300">{new Date(d.lastSeenAt).toLocaleString('hu-HU')}</span></p>}
              </div>
            </div>
          ))}
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-white mb-4">Új PLC Eszköz</h2>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">IP cím</label>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-800 text-gray-300 rounded-lg text-sm">Mégse</button>
              <button onClick={handleSave} className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg text-sm font-medium">Mentés</button>
            </div>
          </div>
        </div>
      )}
