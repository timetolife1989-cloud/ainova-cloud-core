...
export default function ImportConfigsPage() {
  ...
  const handleSave = async () => {
    if (!formName || !formTargetTable) {
      setError(t('admin.import.name_table_required'));
      return;
    }

    const validMappings = formMappings.filter(m => m.source && m.target);
    if (validMappings.length === 0) {
      setError(t('admin.import.column_required'));
      return;
    }
  ...
  const handleDelete = async (config: ImportConfig) => {
    if (!confirm(t('admin.import.confirm_delete', { name: config.configName }))) return;
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <DashboardSectionHeader title={t('admin.import.title')} subtitle={t('admin.import.subtitle')} />
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <DashboardSectionHeader title={t('admin.import.title')} subtitle={t('admin.import.subtitle_full')} />
      ...
      <div className="mt-6 flex justify-end">
        ...
        >
          <Plus className="w-4 h-4" /> {t('admin.import.new_config')}
        </button>
      </div>
      ...
      <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {config.fileType.toUpperCase()} → {config.targetTable} • {t('admin.import.column_count', { count: String(config.columnMapping.length) })}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {config.isActive && (
                <span className="px-2 py-1 bg-green-900/30 text-green-400 text-xs rounded">{t('common.active')}</span>
              )}
              <button onClick={() => openEdit(config)} className="p-2 hover:bg-gray-800 rounded-lg" title={t('common.edit')}>
                <Pencil className="w-4 h-4 text-gray-400" />
              </button>
              <button onClick={() => handleDelete(config)} className="p-2 hover:bg-gray-800 rounded-lg" title={t('common.delete')}>
        ...
        {configs.length === 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
            ...
            <p className="text-gray-400">{t('admin.import.no_configs')}</p>
            <p className="text-gray-500 text-sm mt-1">{t('admin.import.no_configs_hint')}</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-3xl p-6 my-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-white">
                {editingConfig ? t('admin.import.edit_config') : t('admin.import.new_import_config')}
              </h3>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1">{t('admin.import.config_name')}</label>
                  <input
                    type="text"
                    value={formName}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1">{t('admin.import.file_type')}</label>
                  <select
                    value={formFileType}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">{t('admin.import.target_table')}</label>
                <input
                  type="text"
              ...
              {/* Column mappings */}
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-2">{t('admin.import.column_mapping')}</label>
                ...
                >
                  + {t('admin.import.add_column')}
                </button>
              </div>

              {/* Filters */}
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-2">{t('admin.import.filters_optional')}</label>
                <div className="space-y-2">
